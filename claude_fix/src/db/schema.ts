...
export const customerDebtSettlements = pgTable("customer_debt_settlements", {
  id: serial("id").primaryKey(),
  customerId: integer("customer_id").references(() => customers.id).notNull(),
  saleId: integer("sale_id").references(() => sales.id),
  amount: decimal("amount", { precision: 10, scale: 2 }).notNull(),
  paymentMethod: paymentComponentEnum("payment_method").notNull().default("cash"),
  notes: text("notes"),
  userId: integer("user_id").references(() => users.id).notNull(),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});
...
export type CustomerDebtSettlement = typeof customerDebtSettlements.$inferSelect;
