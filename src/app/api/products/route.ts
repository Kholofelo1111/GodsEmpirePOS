...
import { getCurrentUser, sanitizeProductsForRole } from "@/lib/auth";

async function isBarcodeTaken(code: string): Promise<boolean> {
  const existing = await db.select().from(products).where(eq(products.barcode, code));
  console.log("BARCODE CHECK:", code, existing);
