...
function resolvePayments(
  ...
  legs: PaymentLeg[];
  overallMethod: "cash" | "card" | "split" | "eft" | "voucher";
  amountPaid: number;
  ...
  if (rawPayments && rawPayments.length > 0) {
    ...
    const method: PaymentComponent =
      body.paymentMethod === "card"
        ? "card"
        : body.paymentMethod === "eft"
        ? "eft"
        : body.paymentMethod === "voucher"
        ? "voucher"
        : "cash";
    const explicitAmount =
      body.amountPaid !== undefined
        ? Number(body.amountPaid)
        : body.amountTendered !== undefined
        ? Number(body.amountTendered)
        : Number(total);
    legs = [{ method, amount: explicitAmount }];
  }
  ...
  // If customer paid less than total (allowing customer debt / outstanding balance):
  if (sum < Number(total) - 0.01) {
    if (!body.customerId && !body.allowDebt) {
      throw new InvalidPaymentError(
        "A registered customer is required to record an outstanding balance / customer debt."
      );
    }
  } else if (sum - Number(total) > 100000) {
    throw new InvalidPaymentError("Payment amount is unreasonably large");
  }

  const amountPaid = Math.min(Number(total), sum);
  const outstandingBalance = Math.max(0, Number(total) - sum);
  ...
  return { legs, overallMethod, amountPaid, outstandingBalance };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ({ legs, overallMethod, amountPaid, outstandingBalance } = resolvePayments(body, total));
    } catch (err) {
    ...
    const sale = await db.transaction(async (tx) => {
      ...
      const [createdSale] = await tx
        ...
        .values({
          ...
          paymentStatus: outstandingBalance > 0 ? "partial" : "paid",
        })
        .returning();
        ...
        .set({
          ...
          outstandingBalance:
            outstandingBalance > 0
              ? sql`${customers.outstandingBalance} + ${outstandingBalance}`
      ...
      if (outstandingBalance > 0) {
        await tx.insert(notifications).values({
          ...
          message: `Sale ${receiptNumber} created R ${outstandingBalance.toFixed(2)} debt for customer #${resolvedCustomerId}.`,
        });
      }
